...
import { percentBarChartOption, barSeriesOpt, pieChartOption, lineChartOption, lineSeriesOpt } from '@components/customer/dashBoard/chartOptions';
const { TabPane } = Tabs;
...
class TabsAndChartCard extends Component<Props, State> {
  ...
  renderChart = (): void => {
    ...
    setTimeout(() => {
      ...
      this.chart = echarts.init(dom, null, { height: '410px' });
  ...
  render() {
    const { id, size, titleTabList = [{ title: '', code: '' }], chartType, canShowAllData } = this.props;
    ...
    return (
      <Fragment>
        <div className={`base-card ${sizeMap[size]}`}>
          ...
          }
          {
            isForm ?
              formHTML :
              <Fragment>
                {canShowAllData && <a onClick={this.showAllData}>查看完整排行</a>}
        ...
        <Modal
          title="Basic Modal"
          visible={this.state.showAllDataChart}
          // onOk={this.handleOk}
          onCancel={this.hideModal}
          footer={<Button type="primary" onClick={this.hideModal}>关闭</Button>}
        >
          <p>Some contents...</p>
          <p>Some contents...</p>
          <p>Some contents...</p>
